...
class geospatial_resource_Admin(ExtraButtonsMixin, SimpleHistoryAdmin):
    ...
    def get_examples(self, request):  # , queryset):
        ...
        metadataids = [
            "ce253e42-c7e7-11df-94c5-00270e07db9f",
            "a5569dda-c7e7-11df-819e-00270e07db9f",
            "7b22c2aa-c7e7-11df-b1f5-00270e07db9f",
        ]
        ...
        for rec in cswClient.records:
            ...
            xml_file_name = os.path.join(
                settings.MEDIA_ROOT, "repository", f"{new_resource.metadata_id}.xml"
            )
            tif_file_name = os.path.join(
                settings.MEDIA_ROOT, "repository", f"{new_resource.metadata_id}.tif"
            )
            pdf_file_name = os.path.join(
                settings.MEDIA_ROOT, "repository", f"{new_resource.metadata_id}.pdf"
            )

            new_resource.metadata_file.save(
                xml_file_name, ContentFile(metadata.xml.decode("UTF-8"))
            )
    ...
    @admin.action(
        description="Publish selected resources on CSW.", permissions=["publish"]
    )
    def authorize_publish_csw(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{published_count} of {to_publish_count} products published.",
            messages.SUCCESS,
        )

    @admin.action(
        description="Unpublish selected resources on CSW.", permissions=["publish"]
    )
    def unpublish_csw(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{published_count} of {to_publish_count} products unpublished.",
            messages.SUCCESS,
        )
